;(function () {
  ...
  d3.json("data_normalised.json", function (error, jsonData) {
    ...
    // Process your data here for the first city
    var dataForRadarChart = processDataForRadarChart(jsonData[0])

    // Call the RadarChart function with processed data
    RadarChart(".radarChart", [dataForRadarChart], radarChartOptions)
  })
  ...
  window.updateRadarChart = function(selectedLabel) {
    d3.json("data_normalised.json", function(error, jsonData) {
        if (error) throw error;
        
        // Trouver la donnée correspondant au libellé sélectionné
        var selectedItem = jsonData.find(item => item.Libellé === selectedLabel);
        
        if (selectedItem) {
            var dataForRadarChart = processDataForRadarChart(selectedItem);
            
            // Supprimer le radar existant
            d3.select("#radarChart").select("svg").remove();
            
            // Redessiner le radar avec la nouvelle donnée
            RadarChart("#radarChart", [dataForRadarChart], radarChartOptions);
        }
    });
}
